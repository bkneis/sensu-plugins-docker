...
Gem::Specification.new do |s|
  ...
  s.description            = 'This plugin provides native Docker instrumentation
                              ...
                              container status, container number, container
                              metrics via `docker ps`, service status and service logs'
  s.email                  = 'bryan@veeasystems.com'
  s.executables            = Dir.glob('bin/**/*.rb').map { |file| File.basename(file) }
  s.files                  = Dir.glob('{bin,lib}/**/*') + %w(LICENSE README.md CHANGELOG.md)
  s.homepage               = 'https://github.com/bkneis/sensu-plugins-docker'
  s.license                = 'MIT'
  s.metadata               = { 'maintainer'         => '@bkneis',
                               'development_status' => 'active',
                               'production_status'  => 'unstable - testing recommended',
                               'release_draft'      => 'false',
                               'release_prerelease' => 'false' }
  s.name                   = 'sensu-plugins-docker-swarm'
  s.platform               = Gem::Platform::RUBY
  s.post_install_message   = 'You can use the embedded Ruby by setting EMBEDDED_RUBY=true in /etc/default/sensu'
  s.require_paths          = ['lib']
  s.required_ruby_version  = '>= 2.0.0'
  s.summary                = 'Sensu plugins for docker swarm'
